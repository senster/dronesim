...
class AIDrone(Actor):
    ...
    """
    def __init__(self, lat, long, scan_radius, min_lat, max_lat, min_long, max_long, step_size=1.0, drone_id=None):
        """
        Initialize an AI drone with dynamic path planning.
        
        Args:
            lat (float): Initial latitude
            long (float): Initial longitude
            ...
            step_size (float): Distance the drone moves in each step
    ...
    def step(self, ocean_map):
        ...
        for i in range(num_points):
            ...
            lat = self.lat + self.scan_radius * math.cos(angle)
            long = self.long + self.scan_radius * math.sin(angle)
    ...
    def update_drone_positions(self, drones):
        ...
        for drone in drones:
            if drone.drone_id != self.drone_id and isinstance(drone, AIDrone):
                ...
                self.other_drone_positions[drone.drone_id] = (drone.lat, drone.long)
    ...
    def _initialize_density_map(self):
        ...
        """
        # Add a few random points to the density map
        for _ in range(10):
            grid_x = random.randint(int(self.min_lat / self.grid_size), int(self.max_lat / self.grid_size) - 1)
            grid_y = random.randint(int(self.min_long / self.grid_size), int(self.max_long / self.grid_size) - 1)
            self.density_map[(grid_x, grid_y)] = random.uniform(0.1, 0.3)  # Low initial density values
    ...
    def _update_memory(self, current_density):
        ...
        self.scan_memory.append((self.lat, self.long, current_density))
        ...
        grid_x = int(self.lat / self.grid_size)
        grid_y = int(self.long / self.grid_size)
    ...
    def _calculate_sector_boundaries(self):
        ...
        mid_long = (self.min_long + self.max_long) / 2
    ...
    def _identify_clusters(self, current_density):
        ...
        current_grid_x = int(self.lat / self.grid_size)
        current_grid_y = int(self.long / self.grid_size)
        ...
        if current_density >= self.min_cluster_density:
            ...
            for cluster_id, (center_lat, center_long, last_density, _) in list(self.active_clusters.items()):
                ...
                if grid_distance <= self.cluster_radius:
                    ...
                    new_center_lat = center_lat * (1 - weight) + self.lat * weight
                    new_center_long = center_long * (1 - weight) + self.long * weight
                    
                    # Update with higher density if found
                    new_density = max(current_density, last_density)
                    
                    self.active_clusters[cluster_id] = (new_center_lat, new_center_long, new_density, 0)
            ...
            if not updated_existing:
                ...
                self.active_clusters[new_cluster_id] = (self.lat, self.long, current_density, 0)
        ...
        for cluster_id, (center_lat, center_long, density, steps) in self.active_clusters.items():
            ...
            for other_id, (other_lat, other_long, _, _) in other_drone.active_clusters.items():
                ...
                if distance < self.cluster_radius * self.grid_size:
                    ...
                    my_distance = math.sqrt((self.lat - center_lat)**2 + (self.long - center_long)**2)
                    other_distance = math.sqrt((other_drone.lat - center_lat)**2 + (other_drone.long - center_long)**2)
    ...
    def _plan_exploration(self):
        ...
        for angle in angles:
            ...
            new_lat = self.lat + current_step_size * math.cos(angle)
            new_long = self.long + current_step_size * math.sin(angle)
            
            # Check if the new position is within bounds
            if not (self.min_lat <= new_lat <= self.max_lat and self.min_long <= new_long <= self.max_long):
                continue
            
            # Calculate grid coordinates for the new position
            new_grid_x = int(new_lat / self.grid_size)
            new_grid_y = int(new_long / self.grid_size)
            ...
            sector_center_lat = (sector['min_lat'] + sector['max_lat']) / 2
            sector_center_long = (sector['min_long'] + sector['max_long']) / 2
            
            # Calculate vector toward sector center
            sector_dx = sector_center_lat - self.lat
            sector_dy = sector_center_long - self.long
            ...
            for other_drone_id, (other_lat, other_long) in self.other_drone_positions.items():
                # Calculate distance to other drone
                drone_dx = other_lat - new_lat
                drone_dy = other_long - new_long
        ...
        if direction_scores:
            ...
            # Set target position
            self.target_lat = self.lat + current_step_size * math.cos(best_angle)
            self.target_long = self.long + current_step_size * math.sin(best_angle)
            
            # Ensure target is within bounds
            self.target_lat = max(self.min_lat, min(self.max_lat, self.target_lat))
            self.target_long = max(self.min_long, min(self.max_long, self.target_long))
        else:
    ...
    def _plan_exploitation(self, ocean_map):
        ...
        for cluster_id in self.assigned_clusters:
            if cluster_id in self.active_clusters:
                ...
                for other_drone_id, (other_lat, other_long) in self.other_drone_positions.items():
                    other_dx = center_lat - other_lat
                    other_dy = center_long - other_long
        ...
        if best_cluster_id is not None:
            ...
            # Set target position
            self.target_lat = self.lat + step_size * math.cos(angle)
            self.target_long = self.long + step_size * math.sin(angle)
    ...
    def _plan_forced_exploration(self):
        ...
        # Set target position
        self.target_lat = self.lat + escape_step_size * math.cos(new_angle)
        self.target_long = self.long + escape_step_size * math.sin(new_angle)
    ...
    def _plan_random_move(self):
        ...
        # Set target position
        self.target_lat = self.lat + self.step_size * math.cos(angle)
        self.target_long = self.long + self.step_size * math.sin(angle)
    ...
    def _move_to_target(self):
        ...
        """
        if self.target_lat is not None and self.target_long is not None:
            self.lat = self.target_lat
            self.long = self.target_long
